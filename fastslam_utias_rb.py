...
def run_SLAM(config, plot=False, seed=None):
    ...
    if not plot:
        fig, ax = plt.subplots()
        plot_history(ax, dead_reckoning[::100], color='purple')
        plot_history(ax, stats.ground_truth_path[::100], color='green')
        plot_history(ax, stats.predicted_path[::100], color='orange')
        ...
        plt.savefig(f"figs_utias/{seed}.png")
