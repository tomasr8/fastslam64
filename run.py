...
if __name__ == "__main__":
    ...
    # from config_jacobian_circle import config
    # from fastslam_known import run_SLAM
    from config_fsonline_direct import config
    from fastslam_fsonline_direct import run_SLAM

    context.set_limit(limit.MALLOC_HEAP_SIZE, 2 * config.GPU_HEAP_SIZE_BYTES)

    # for N, THREADS in zip([32, 64, 128, 256, 512, 1024], [32, 64, 128, 256, 512, 512]):
    for N, THREADS in zip([4, 8, 16], [4, 8, 16]):
        config.N = N
        ...
        fname = f"figs_fsonline/fixed_result_{config.N}_{config.THRESHOLD}.json"
